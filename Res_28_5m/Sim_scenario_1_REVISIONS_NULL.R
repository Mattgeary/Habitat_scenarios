...
hab.3 <- focal(hab.3, w = matrix(c(rep(1, 5041)), nrow = 71), mean, na.rm=T, pad=T)
setwd(output.1)
...
hab.4 <- focal(hab.4, w = matrix(c(rep(1, 5041)), nrow = 71), mean, na.rm=T, pad=T)
setwd(output.1)
...
hab.2 <- focal(hab.2, w = matrix(c(rep(1, 5041)), nrow = 71), mean, na.rm=T, pad=T)
setwd(output.1)
...
writeRaster(pred, filename=(paste(getwd(), "Scenario_1", paste("pred_map_REVISIONS_NULL", j, ".grd", sep= ""), sep="/")), overwrite=T)
rm(env.l.new)
gc()
...
write.csv(pres.abs, "Scenario_1_REVISIONS_NULL.csv", row.names=F)
setwd(work)