...
gc()
output.1 <- paste(getwd(), "Scenario_1/Revisions_AIC", sep="/")
...
writeRaster(pred, filename=(paste(getwd(), "Scenario_1/Revisions_AIC", paste("pred_map_REVISIONS_AIC", j, ".grd", sep= ""), sep="/")), overwrite=T)
rm(env.l.new)
gc()
...
write.csv(pres.abs, "Scenario_1_REVISIONS_AIC.csv", row.names=F)
setwd(work)