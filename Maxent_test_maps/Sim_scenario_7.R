...
for(i in 1:iterations){
	if(cellStats(new.hab.1, sum) <= 1470.35) new.hab.1 <- f.Exp(new.hab.1, potential.pts, p.trans)
...
for(i in 1:iterations){
	if(cellStats(new.hab.2, sum) <= 1943.97) new.hab.2 <- f.Exp(new.hab.2, potential.pts, p.trans)
...
write.csv(pres.abs, "Scenario_7.csv")
