gc()
output.3 <- paste(getwd(), "goodhab", sep="/")
...
for(i in 1:iterations){
	if(cellStats(new.hab.1, sum) <= 3239.95) new.hab.1 <- f.Exp(new.hab.1, potential.pts, p.trans)
...
for(i in 1:iterations){
	if(cellStats(new.hab.2, sum) <= 3239.95) new.hab.2 <- f.Exp(new.hab.2, potential.pts, p.trans)
	else new.hab.2 <- new.hab.2
    }

dir.create(paste(getwd(), "/goodhab/run_", j, sep=""))
...
hab.4 <- focal(hab.4, w=17, mean, na.rm=T, pad=T)
setwd(output.3)
...
writeRaster(pred, filename=(paste(getwd(), "goodhab", paste("pred_map", j, ".grd", sep= ""), sep="/")), overwrite=T)
...
write.csv(pres.abs, "goodhab.csv")
