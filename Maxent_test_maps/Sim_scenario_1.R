...
for(i in 1:iterations){
	if(cellStats(new.hab.1, sum) <= 1260.3) new.hab.1 <- f.Exp(new.hab.1, potential.pts, p.trans)
...
for(i in 1:iterations){
	if(cellStats(new.hab.2, sum) <= 840.2) new.hab.2 <- f.Exp(new.hab.2, potential.pts, p.trans)
...
write.csv(pres.abs, "Scenario_1.csv")
