...
for(i in 1:iterations){
	if(cellStats(new.hab.1, sum) <= 4535.93) new.hab.1 <- f.Exp(new.hab.1, potential.pts, p.trans)
...
for(i in 1:iterations){
	if(cellStats(new.hab.2, sum) <= 354.72) new.hab.2 <- f.Exp(new.hab.2, potential.pts, p.trans)
...
hab.5 <- focal(hab.5, w=17, mean, na.rm=T, pad=T)
setwd(output.6)
...
write.csv(pres.abs, "Scenario_6.csv")
